...
<form class="bg-slate-200 w-4/5 p-[60px] rounded-md " method="POST" action="AdminCenterList">
  <div class="grid gap-6 mb-6 md:grid-cols-2">
    ...
    <div>
      ...
      <select id="city" name="idcity" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
        <c:forEach items="${Allcity}" var="city">
        <option value="${city.getId()}"><c:out value="${city.getCname()} in :  ${city.getCville()}"/></option>
        </c:forEach>
      </select>
    </div>
  </div>
